...
# 📥 Arquivo de entrada e pasta de saída
...
while True:
    ...
    if opcao == "1":
        ...
        for conteudo, metrica in metricas_tempo_total.items():
            tempo_formatado = converter_segundos_para_hms(metrica['tempo_total_visualizacao'])
            nome_conteudo = metrica['nome_conteudo']
        ...
        for conteudo, metrica in metricas_media_tempo.items():
            nome_conteudo = metrica['nome_conteudo']
            tempo_formatado = converter_segundos_para_hms(metrica['media_tempo_visualizacao'])
            print(f"📺 {conteudo} - {nome_conteudo}: {tempo_formatado}")
